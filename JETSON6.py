...
Camera notes (IMX708 specific quirks):
  ...
  - Brightness can be adjusted at two levels:
      1. Argus auto-exposure compensation (real exposure change).
      2. Software HSV V-channel multiplier (after-capture).
...
# Argus auto-exposure compensation. -2.0 (much darker) ... +2.0 (much brighter).
# Real exposure change at the sensor level — preferred over software brightness.
CAMERA_EXPOSURE_COMP = 1.0

# IMX708 driver desaturates; boost in HSV. 1.0 = no change. 1.6 ≈ +60%.
SATURATION_BOOST = 1.6

# Software brightness multiplier (HSV value channel). 1.0 = no change.
# Use this for fine-tuning AFTER the Argus exposure_comp gets you in the ballpark.
...
def _build_camera_pipeline() -> str:
    ...
    to 1280x720 BEFORE the frame reaches Python.
    """
    return (
    ...
    def camera_worker(self) -> None:
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                # IMX708 driver delivers desaturated colors; boost saturation
                # AND brightness in HSV in the same pass.
