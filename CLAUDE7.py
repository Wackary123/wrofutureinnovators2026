...
- Mid-speech utterances must pass a slightly STRICTER noise gate
  (MID_SPEECH_MIN_CONF = 0.65) to reduce false interrupts from any stray
...
# Slightly stricter to reduce false interrupts. Raise if helmet interrupts
# itself, lower if real interrupts get ignored.
MID_SPEECH_MIN_CONF = 0.65
MID_SPEECH_MIN_WORDS = 3

# --- Self-hearing guard (tiny settle only) ---
# User has confirmed speaker/mic are physically far apart. This settle just
# covers the brief tail when aplay is terminating.
...
"""
    ...
    def _estimate_spoken_portion(self, full_text: str,
                                 speak_start: float,
                                 cut_time: float) -> str:
        """Estimate how much of `full_text` was audible before the cut.

        We use a simple rate model: ~13 chars/sec at length_scale=1.0, scaled
        by PIPER_LENGTH_SCALE. Then we truncate at the nearest sentence or
        word boundary.
        """
        if not full_text:
            return ""
        elapsed = max(0.0, cut_time - speak_start)
        # Approximate chars-per-second. Faster length_scale = more chars/sec.
        ...
        if audible_chars >= len(full_text):
            return full_text
        # Prefer to cut at a sentence boundary before audible_chars, else word.
    ...
    def _handle_request(self, kind: str, text: str) -> None:
        ...
        # Phase 1 — generation. Cancel here just drops the request.
        ...
        # Phase 2 — playback. Cancel here saves a partial reply.
        print(f"🤖 {response}")
        self.is_busy_event.set()
        self.speak_start_time = time.time()
    ...
    def _listen_forever(self) -> None:
        ...
        def audio_callback(indata, frames, time_info, status):
            # Small settle window right after speaking ends.
                ...
                with sd.RawInputStream(
                    ...
                    while not self.stop_event.is_set():
                        ...
                        if utt_start is None:
                            utt_start = time.time()
                            # Tag whether we were speaking when this utterance started.
    ...
    def start(self) -> None:
        ...
        try:
            while not self.stop_event.is_set():
                ...
                if self.is_busy_event.is_set():
                    # Hard-cut current response. The partial reply will be
                    # saved to memory by _handle_request's finally block.
                    print("[Interrupt] cutting current response")
                    self.cancel_response_event.set()
                    self._hard_stop_all_audio()
                    # Small pause so the worker's finally block runs and
                    # memory gets committed before we enqueue the new turn.
                    time.sleep(0.10)
