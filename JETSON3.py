...
Hardware:
  ...
  - Raspberry Pi Camera Module 3 (IMX708) on /dev/video0
  - USB MillSO mic on sounddevice index 1 (hw:1,0)
  - USB speaker on plughw:0,0 (UACDemoV1.0)

Behavior summary (carried over from Pi version):
  - Greeting on startup
  - Continuous STT, but drops audio while speaking (self-hearing guard)
  ...
  - Camera triggers for vase / sword / mona lisa / pharaoh mask, gated
    by hold time + cooldown + confidence
...
# --- Audio input (mic) ---
MIC_DEVICE = 1                  # sounddevice index of USB mic. Verify with sd.query_devices().
...
PIPER_LENGTH_SCALE = 1.00       # 1.0 default; lower = faster, higher = slower
...
# --- Vision ---
DETECT_EVERY_N_FRAMES = 2       # Lower than Pi version because Jetson GPU is much faster
...
CAMERA_CAPTURE_SIZE = (1280, 960)   # what we ask V4L2 to deliver
CAMERA_PROCESS_SIZE = (1024, 1024)  # what YOLOE actually sees (after resize)
YOLOE_IMGSZ = 320                   # GPU lets us bump this back up vs Pi
...
class MuseumHelmet:
    def __init__(self):
        ...
        # We hold off loading the actual YOLOE model until camera_worker
        # because it's a big alloc and we want startup logs to be in order.
...
"""
    ...
    # --------------------------------------------------------------------
    # Camera worker (V4L2 OpenCV path, YOLOE on CUDA).
    # --------------------------------------------------------------------
    def camera_worker(self) -> None:
        ...
        try:
            ...
            print(f"[Camera] Opened {CAMERA_DEVICE}, frame shape: {test_frame.shape}")
            ...
            while not self.stop_event.is_set():
                ...
                try:
                    ...
                    # Headless — write latest frame to disk for inspection.
